...
function drawBoard() {
  requestAnimationFrame(function gameLoop() {
    ...
    if (!currentShape || currentShape.active === false) {
      ...
      // check currentShape for left side of the board
      if ( (currentShape.piece.shape[4].x < board.columns &&
            currentShape.piece.shape[3].x < board.columns &&
            currentShape.piece.shape[2].x < board.columns &&
            currentShape.piece.shape[1].x < board.columns) &&

      // check currentShape for right side of the board
           (currentShape.piece.shape[4].x < board.columns &&
            currentShape.piece.shape[3].x < board.columns &&
            currentShape.piece.shape[2].x < board.columns &&
            currentShape.piece.shape[1].x < board.columns))
